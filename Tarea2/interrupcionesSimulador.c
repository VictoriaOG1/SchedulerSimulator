...
int calcular_50_50(int num_procesos) {
    int index50;
    index50 = (num_procesos/2) - 1;
    return index50;
}

//ordenar aleatoriamente los procesos
void shuffle(struct Process* arreglo, int n){
    int indice_aleatorio;
    struct Process temporal;
    for (int i = n - 1; i > 0; i--) {
    ...
    while (1)
    {
        ...
        for (i = 0; i < n; i++)
        {
            ...
            if (pro[i].arrivalTime <= time)
            {
                ...
                pro[i].state = 1;
                int typeBound = pro[i].typeBound;
                
                if(typeBound == 0){ //CPU bound
                    //calcula la probabilidad de interrupcion
                    positionInteQueue = rand() % 3;
                    
                    if(interruptsQueue[positionInteQueue] == 0){
                        //interuptir el proceso mandarlo a la cola de bloqueado y calcular el tiempo de interrupcion
                        blockTime = rand(0,5);
                    }
                }
                if(typeBound == 1){ //IO bound
                    //calcula la probabilidad de interrupcion
                    positionInteQueue = rand() % 3;
...
int main()
{
    ...
    //literal a - 50/50:
    int proIndex = calcular_50_50(n);
    
    //literal b y c - 90/10, 10/90:
    //int proIndex = calcular_90_10(n);
    
    //ordenar el arreglo de forma aleatoria para escoger los procesos de forma aleatoria
    shuffle(pro, n);
    
    //asignar que los procesos sean CPU o IO (literal a y b)
    for(int i = 0; i < n; i++){
        if(i<= proIndex){
            pro[i].typeBound = 0; //CPU bound
        }
        else{
            pro[i].typeBound = 1; //IO bound
        }
    }
    
    //asignar que los procesos sean CPU o IO (literal c)
    //for(int i = 0; i < n; i++){
        //if(i<= proIndex){
            //pro[i].typeBound = 1; //CPU bound
        //}
        //else{
            //pro[i].typeBound = 0; //IO bound
        //}
    //}
